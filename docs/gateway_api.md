# Phiner Gateway API 文档 (Redis Stream & Pub/Sub)
...
## 1. 存储与流 (Storage & Streams)

Gateway 会将行情流和全量状态写入以下 Redis 键中。

| 键名 (Key) | 数据结构 (Type) | 格式 | 描述与使用场景 |
| :--- | :--- | :--- | :--- |
| `gateway:ticks:stream:{Instrument}` | `Stream` | Shorthand Map | **实时 Tick 流**。采用极简字段名，各品种独立 Stream。容量限制 20k-30k。 |
| `gateway:kline:{Instrument}:{Period}` | `Sorted Set` | Msgpack `BarDTO` | **K线缓存**。使用时间戳作为 Score。 |
| `gateway:positions:active` | `Hash` | Msgpack `PositionDTO` | **活跃持仓**。Field=`dealId`。 |
| `gateway:orders:history` | `Hash` | Msgpack `OrderHistoryDTO` | **订单历史**。Field=`dealId`。 |
| `gateway:config:instruments` | `Set` | String | **已订阅的外汇品种列表**。 |

### 1.1 Tick Stream 字段说明
为了减少网络开销和提高解析速度，Stream 中的数据不再包含品种名，且字段名缩写如下：
- `t`: 时间戳 (milliseconds)
- `b`: 买入价 (bid)
- `a`: 卖出价 (ask)
- `bv`: 买入量 (bid volume)
...
| :--- | :--- | :--- |
| `gateway:kline:{Instrument}:{Period}` | Msgpack `BarDTO` | K 线走完或更新时的实时推流。 |

> [!NOTE]
> 原有的 `gateway:tick:{Instrument}` Pub/Sub 频道已废弃，请统一迁移至 `gateway:ticks:stream:{Instrument}`。
...
**示例 A：监听实时行情 (Redis Stream)**
推荐使用 `XREAD` 或 `XREADGROUP` 进行消费。

```rust
use redis::{Commands, streams::{StreamReadOptions, StreamReadReply}};
...
fn main() -> redis::RedisResult<()> {
    ...
    let stream_key = format!("gateway:ticks:stream:{}", instrument);
    
    // 从最新一条记录开始读取 ($)
    let mut last_id = "$".to_string();

    println!("正在通过 Stream 监听 {} Tick...", instrument);
    loop {
        let opts = StreamReadOptions::default().count(10).block(5000);
        let result: StreamReadReply = con.xread_options(&[&stream_key], &[&last_id], &opts)?;
        ...
        for stream in result.keys {
            for entry in stream.ids {
                ...
                // 解析缩写字段
                let map: HashMap<String, String> = entry.map;
                let t = map.get("t").unwrap();
                let b = map.get("b").unwrap();
                let a = map.get("a").unwrap();

                println!("Time: {}, Bid: {}, Ask: {}", t, b, a);
            }
        }
    }
}
