...
| :--- | :--- | :--- | :--- |
| `gateway:kline:{Instrument}:{Period}` | `List` | Msgpack `BarDTO` | **K线缓存**。例如 `gateway:kline:EUR/USD:5m`。使用 `LRANGE 0 -1` 获取限定长度（如最近45根）的历史K线。 |
...
### 5.2 Python 接入示例 (需安装 `redis` 和 `msgpack` 包)
...
for message in pubsub.listen():
    if message['type'] == 'message':
        ...
        print(f"[{tick_data['instrument']}] Bid: {tick_data['bid']} Ask: {tick_data['ask']}")
```
...
# 构造请求字典（严格按照 SubmitOrderRequest 的结构）
order_request = {
    "instrument": "GBP/USD",
    "orderCommand": "BUY",
    "amount": 0.1,  # 1 万资金
    "label": "MACD_Strategy_01",
    "comments": "5m",  # 订单备注，记录周期信息
    "price": 0.0,
    "slippage": 5.0,
    "stopLossPrice": 0.0,
...
# 序列化为 Msgpack
packed_data = msgpack.packb(order_request, use_bin_type=True)

# 发布到网关的指令频道
r.publish('gateway:order:submit', packed_data)
print("Order submitted via Redis Pub/Sub.")
```

**示例 C：读取 K 线历史缓存 (List Storage)**
```python
import redis
import msgpack
...
# 获取 EUR/USD 的 15 分钟 K 线列表，拉取所有缓存 de 记录 (0 到 -1)
kline_key = "gateway:kline:EUR/USD:15m"
raw_bars = r.lrange(kline_key, 0, -1)

print(f"Loaded {len(raw_bars)} bars from memory.")
for raw_bar in raw_bars:
    bar = msgpack.unpackb(raw_bar, raw=False)
    print(f"Time: {bar['time']}, Close: {bar['close']}, Vol: {bar['volume']}")
```
...
for message in pubsub.listen():
    if message['type'] == 'message':
        error = msgpack.unpackb(message['data'], raw=False)
...
subscriber.subscribe('gateway:error:structured');

subscriber.on('message', (channel, message) => {
  if (channel === 'gateway:error:structured') {
    const error = msgpack.decode(Buffer.from(message));
